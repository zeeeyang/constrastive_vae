...
x_test = torch.zeros((4 * args.test_num_particles, 4), device='cuda' if gpu else 'cpu') # 4*64, 4
...
print("[tlog] " + str(x_test))
eps_test = torch.zeros((4 * args.test_num_particles, 2), device=x_test.device).normal_(0, 1) # why 2??
...
def kl_approx_loss_logexp(a,b): 
    zero = torch.tensor([0], device=a.device)
...
for epo in pbar:
    ...
    eps2 = torch.zeros((4 * args.num_particles, 2), device=x.device).normal_(0, 1)
    z_x2 = encoder(x, eps2)
    ...
    for k in torch.arange(args.num_nu_updates):
        ...
        z2 = torch.zeros((4 * args.num_particles, 2), device=x.device).normal_(0, 1)
