...
def evaluation(data, model, meta_optimizer):
    ...
    for i in pbar:
        ...
        mean, logvar, _ = model._enc_forward(sents)
        if args.model == 'savae':
        ...
        # active units
        total_mean_au += torch.sum(mean, dim=0).cpu().numpy()
        total_sq_au += torch.sum(mean ** 2, dim=0).cpu().numpy()
...
for epo in torch.arange(epo_0 + 1, args.num_epochs + 1):
    ...
    for bat in pbar:
        ...
        mean, logvar, enc_h = model._enc_forward(sents)

        mean1, logvar1, enc_h1 = model._enc_forward(sents)
        ...
        if args.model == 'vae' or args.model == 'beta_vae' or args.model == 'cyc_vae':
            ...
            z_samples1 = model._reparameterize(mean1, logvar1)
            #print(z_samples.size())
            #sys.exit(0)
            preds1 = model._dec_forward(sents, z_samples1)
            nll_vae1 = sum([criterion(preds1[:, l], sents[:, l + 1]) for l in range(length)])
            kl_vae1 = utils.kl_loss_diag(mean1, logvar1)

            vae_loss += nll_vae1 + beta * kl_vae1
            #vae_loss *= 0.5


            z_x, z_x2 = enc_h, enc_h1 #z_samples, z_samples1 #aug_zx, aug_zx2
            ...
            labels = torch.arange(cos_sim.size(0)).long().to(z_samples.device)
            self_sup_loss = self_sup_loss_fct(cos_sim, labels)

            vae_loss += self_sup_loss
